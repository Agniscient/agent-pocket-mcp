...
interface ApprovalRequest {
  id: string;
  userId: string;
  type: "approval" | "question" | "manual_step" | "info";
  message: string;
  context?: string;
  status: string;
  answer?: string;
...
const SERVER_URL =
  process.env.APPROVAL_SERVER_URL ||
  `http://localhost:${process.env.PORT || DEFAULT_SERVER_PORT}`;
const API_KEY = process.env.APPROVAL_API_KEY || "";
const TIMEOUT_SEC = process.env.APPROVAL_TIMEOUT_SEC
  ? Number(process.env.APPROVAL_TIMEOUT_SEC)
  : undefined;
...
async function createApproval(
  ...
  const body: Record<string, unknown> = { type, message, context };
  if (TIMEOUT_SEC !== undefined) body.ttl = TIMEOUT_SEC;
...
async function pollApproval(id: string): Promise<ApprovalRequest> {
  while (true) {
    ...
    if (approval.status === "timed_out") {
      throw new Error("Request timed out — no response received before the deadline.");
    }
...
const server = new Server(
  { name: "agent-pocket", version: "1.0.0" },
  { capabilities: { tools: {} } }
);

const AGENT_TARGETS: Record<string, { file: string; description: string }> = {
...
const POLLING_RULES = `
## Agent Pocket: Task Polling

This IDE tab is configured as an agent tab. When idle, poll for tasks using this loop:

### Polling loop
- You should not call \`register_as_agent\` if the tab is already registered once.
- Call \`poll_for_task\` when idle.
- It blocks internally until a task arrives — no need to re-call on wait.
- If it returns a task object \`{ id, title, description }\`: execute the task, then call \`complete_task(taskId)\` when done, then poll again.
- If it returns \`{ terminated: true }\`: stop polling. Do not restart unless the user asks.

### Rules
- Never start unsolicited work without polling first.
- Never leave a task incomplete without calling \`complete_task\`.
`.trim();

function generateTabName(): string {
  const base = path.basename(process.cwd()).replace(/^[./]+$/, "") || "pocket-agent";
  ...
  tools: [
    {
      ...
      description:
        "Ask the user to approve or reject an action. Blocks until they respond. " +
    ...
    },
    {
      name: "ask_question",
      description:
        "Ask the user a clarifying question and wait for their text answer. " +
    ...
    },
    {
      ...
      description:
        "Ask the user to perform a manual step you cannot do (e.g. click a button, enter a CAPTCHA, pull a physical cable). " +
    ...
    },
    {
      name: "notify",
      description:
        "Send a non-blocking informational notification to the user — e.g. 'Build complete' or 'Tests passed'. " +
    ...
    },
    {
      ...
      description:
        "Initialize Agent Pocket human-in-the-loop approval rules in a project's AI agent config files. " +
        "Writes request_approval / ask_question / notify rules. " +
        "For agent task polling rules use init_agent_polling instead. " +
      ...
      inputSchema: {
        ...
        properties: {
          ...
          targets: {
            ...
            items: { type: "string" },
            description:
              "Which agent config files to update. Options: cursor, windsurf, claude, copilot, cline. " +
              "Defaults to all.",
          },
        },
        required: [],
    ...
    {
      name: "init_agent_polling",
      description:
        "Add agent task-polling rules to this project's AI agent config files. " +
        "Writes poll_for_task / complete_task loop instructions as a separate rules block. " +
        "Run this in addition to init_pocket_agent if this IDE tab should act as an agent. " +
        "You MUST follow the returned instructions and create/edit the files as directed.",
      inputSchema: {
        type: "object" as const,
        properties: {
          ...
          targets: {
            type: "array",
            items: { type: "string" },
            description: "Which agent config files to update. Options: cursor, windsurf, claude, copilot, cline. Defaults to all.",
          },
        },
        required: [],
      },
    },
    {
      ...
      description:
        "One-time registration for this IDE tab as an agent. " +
        "Call this once when the tab starts (before polling). " +
        "The tab name is auto-generated from the project directory and persisted locally — stable across restarts. " +
        "Returns { agentId, name }. Store agentId in context and pass it to every poll_for_task call.",
      inputSchema: {
        type: "object" as const,
        properties: {
          ideType: {
            type: "string",
            enum: ["cursor", "windsurf", "claude", "copilot", "cline"],
            description: "The IDE or tool this tab is running in. Defaults to 'windsurf'.",
          },
        },
        required: [],
      },
    },
    {
      name: "poll_for_task",
      description:
        "Wait for a task to be assigned to this agent tab. " +
        "Pass the agentId returned by register_as_agent. " +
        "Blocks internally until a task is available, then returns it. " +
        "Call this when idle and again after completing each task. " +
        "Returns the task object when available, or { terminated: true } if polling was stopped from the dashboard.",
      inputSchema: {
        type: "object" as const,
        properties: {
          agentId: {
            type: "string",
            description: "The agentId returned by register_as_agent. Required.",
          },
          ideType: {
            type: "string",
            enum: ["cursor", "windsurf", "claude", "copilot", "cline"],
    ...
    },
    {
      name: "complete_task",
      description:
        "Mark the current task as done. Always call this when you finish a task. " +
        "Pass the taskId returned by poll_for_task. " +
        "Returns { ok: true, task } on success.",
      inputSchema: {
        type: "object" as const,
        properties: {
          taskId: { type: "string", description: "The task ID returned by poll_for_task" },
        },
        required: ["taskId"],
      },
    },
...
function applyRulesToFile(
  ...
  if (fs.existsSync(filePath)) {
    ...
    // Already up-to-date
    if (existing.includes(rules)) return "skipped";

    // Find the earliest occurrence of any sentinel matching this block type
    const idx = sentinels
    ...
    if (idx !== Infinity) {
      // Replace from the sentinel to the next double-newline-separated block boundary
      // Find end of this block: next sentinel from the OTHER block type, or EOF
      const otherSentinels = sentinels === POLLING_SENTINELS ? APPROVAL_SENTINELS : POLLING_SENTINELS;
      const endIdx = otherSentinels
        .map((s) => existing.indexOf(s, idx + 1))
        .filter((i) => i !== -1)
        .reduce((min, i) => Math.min(min, i), Infinity);

      const before = existing.slice(0, idx).trimEnd();
      const after = endIdx !== Infinity ? "\n\n" + existing.slice(endIdx).trimStart() : "";
      fs.writeFileSync(filePath, (before ? before + "\n\n" : "") + rules + after + "\n");
    } else {
      // No existing block of this type — append
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  if (request.params.name === "request_approval") {
    const { message, context } = request.params.arguments as { message: string; context?: string };
    try {
  ...
  if (request.params.name === "register_as_agent") {
    const args = request.params.arguments as { ideType?: string };
    if (!API_KEY) return { content: [{ type: "text", text: "Error: APPROVAL_API_KEY not set." }], isError: true };
    try {
      const tabName = generateTabName();
      const res = await fetch(`${SERVER_URL}/api/agent/register`, {
        method: "POST",
        headers: headers(),
        body: JSON.stringify({ name: tabName, ideType: args.ideType ?? "windsurf" }),
      });
      if (res.status === 401) return { content: [{ type: "text", text: "Error: Invalid API key." }], isError: true };
      if (res.status === 403) return { content: [{ type: "text", text: "Error: Agent polling is not enabled. Enable it in Agent Pocket settings." }], isError: true };
  ...
  if (request.params.name === "poll_for_task") {
    const args = request.params.arguments as { agentId?: string; ideType?: string };
    const agentId = args.agentId;
    if (!agentId) return { content: [{ type: "text", text: "Error: agentId is required. Call register_as_agent first." }], isError: true };
    if (!API_KEY) return { content: [{ type: "text", text: "Error: APPROVAL_API_KEY not set." }], isError: true };
    // Loop internally so the AI only wakes up when there is a real task or termination.
    while (true) {
      try {
        const res = await fetch(`${SERVER_URL}/api/agent/poll-task`, {
          method: "POST",
          headers: headers(),
          body: JSON.stringify({ agentId, ideType: args.ideType ?? "windsurf" }),
        });
        if (res.status === 401) return { content: [{ type: "text", text: "Error: Invalid API key." }], isError: true };
        if (res.status === 403) return { content: [{ type: "text", text: "Error: Agent polling is not enabled. Enable it in Agent Pocket settings." }], isError: true };
        const data = await res.json() as any;
        if (data.wait) continue; // no task yet — loop immediately
        return { content: [{ type: "text", text: JSON.stringify(data) }] };
      } catch (err: any) {
        return { content: [{ type: "text", text: `Error: ${err.message}` }], isError: true };
      }
  ...
  if (request.params.name === "complete_task") {
    ...
    if (!API_KEY) return { content: [{ type: "text", text: "Error: APPROVAL_API_KEY not set." }], isError: true };
    try {
      const res = await fetch(`${SERVER_URL}/api/agent/complete-task`, {
        method: "POST",
        headers: headers(),
      ...
      if (!res.ok) {
        ...
        return { content: [{ type: "text", text: `Error: ${err.error || res.statusText}` }], isError: true };
      }
  ...
  if (request.params.name === "init_pocket_agent" || request.params.name === "init_agent_polling") {
    const isPoll = request.params.name === "init_agent_polling";
    const { project_path, targets } = request.params.arguments as {
      project_path?: string;
      targets?: string[];
    };
    ...
    if (selectedKeys.length === 0) {
      return {
        content: [{ type: "text", text: "No valid targets. Options: " + Object.keys(AGENT_TARGETS).join(", ") }],
        isError: true,
      };
    }

    let baseRules = isPoll ? null : APPROVAL_RULES;
    const sentinels = isPoll ? POLLING_SENTINELS : APPROVAL_SENTINELS;
    if (!isPoll) {
      try {
        const res = await fetch(`${SERVER_URL}/api/rules`);
        if (res.ok) baseRules = await res.text();
      } catch {
    ...
    for (const key of selectedKeys) {
      ...
      const rules = isPoll ? POLLING_RULES : baseRules!;
      try {
        const outcome = applyRulesToFile(filePath, rules, sentinels);
        const label = outcome === "created" ? "Created" : outcome === "updated" ? "Updated" : "Already up-to-date (skipped)";
    ...
    const prefix = isPoll ? "Agent polling rules written" : "Agent Pocket initialized";
    return {
      content: [{ type: "text", text: `${prefix} in: ${projectRoot}\n\n` + results.join("\n") }],
    };
  }
